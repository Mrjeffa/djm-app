...
const inp = { width:'100%', background:'#1E1E1E', border:`1px solid #2A2A2A`, borderRadius:6, padding:'11px 14px', color:'#F0ECE6', fontSize:15, fontFamily:'sans-serif', outline:'none', boxSizing:'border-box', marginBottom:10 }
const btn = (color='#E8520A') => ({ width:'100%', padding:13, background:color, color:'#fff', border:'none', borderRadius:8, fontSize:15, fontWeight:600, cursor:'pointer', fontFamily:'sans-serif', marginTop:4 })
const card = { width:'100%', maxWidth:380, background:'#161616', border:`1px solid #2A2A2A`, borderRadius:10, padding:28 }

export default function LoginScreen() {
  const [tab, setTab] = useState('login') // login | register | reset_ok
  ...
  const registreer = async () => {
    ...
    setLaden(false)
    if (error) setFout('Fout: ' + error.message)
  }
      ...
      <div style={{ width:'100%', maxWidth:380 }}>
        ...
        {tab === 'reset_ok' ? (
          <div style={{...card, textAlign:'center'}}>
            ...
            <div style={{marginTop:16,fontSize:12,color:T.muted,cursor:'pointer',textDecoration:'underline'}} onClick={()=>setTab('login')}>← Terug</div>
          </div>
        ) : (
          <div style={card}>
            {/* Tab switcher */}
            <div style={{display:'flex',gap:0,marginBottom:20,background:'#1E1E1E',borderRadius:6,padding:3}}>
              {[['login','Inloggen'],['register','Nieuw account']].map(([id,lbl])=>(
                <button key={id} onClick={()=>{setTab(id);setFout(null)}}
                  style={{flex:1,padding:'8px',background:tab===id?T.accent:'transparent',color:tab===id?'#fff':T.muted,border:'none',borderRadius:4,fontSize:13,fontWeight:tab===id?600:400,cursor:'pointer',fontFamily:'sans-serif'}}>
                  {lbl}
                </button>
              ))}
            </div>
